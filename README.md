...
##### Examples
See the `svg` directory for .svg rendered files, _view raw_ for the full image; otherwise look for .png files inside the `pics` directory. _raw_ - stops are mapped onto plane acording to thier geographical coordinates. _optimized_ - we give up geographical accuracy to achieve a better-looking image; stops are uniformly distributed across the plane, and some coordinates are compressed into one.
